...
# 常量定义
MAX_CONTEXT_LENGTH = 20  # 上下文最大消息对数
...
async def show_timeout_options(update: Update,
    ...
    timeout_text += f"当前超时时间: <b>{current_display}</b>\n\n"
...
async def handle_specific_actions(update: Update,
    ...
    if action == "template" and len(parts) >= 2:
        ...
        await query.edit_message_text(
            ...
            f"请输入新服务商的 ID (使用字母和数字):",
            parse_mode="HTML")

        # 设置会话状态，等待用户输入服务商 ID，并在会话状态中包含模板ID
        await session_manager.set(user_id,
                                  "ai_waiting_for",
                                  f"provider_id_{template_id}",
                                  chat_id=chat_id,
...
async def show_provider_edit_menu(update: Update,
    ...
    if query:
        ...
        try:
            ...
            _interface.logger.error(f"发送编辑菜单失败: {str(e)}")
    else:
...
async def show_whitelist_remove_menu(
    ...
    query = update.callback_query
    if not query:
        _interface.logger.error("无法获取回调查询，无法显示白名单移除菜单")
        return
...
async def handle_config_callback(update: Update,
    ...
    elif parts[0] == "whitelist":
        # 默认设置为基本白名单操作
        action = "whitelist"

        # 检查是否是特定的白名单操作
        if len(parts) >= 3:
            if parts[1] == "remove" and parts[2] == "user":
                action = "whitelist_remove_user"
            elif parts[1] == "clear" and parts[2] == "confirm":
    ...
    if action == "view":
        ...
        if len(parts) == 1:
            ...
            if provider_id in _state["providers"]:
                ...
                _interface.logger.warning(
                    f"用户 {user_id} 尝试编辑不存在的服务商: {provider_id}")
                await query.answer(f"服务商 {provider_id} 不存在")
                await show_provider_list(update, context, "edit")
    ...
    elif action == "whitelist" or action == "whitelist_remove_user" or action == "whitelist_clear_confirm":
        # 将所有白名单相关操作委托给 handle_specific_actions 处理
        ...
        waiting_for = await session_manager.get(user_id,
                                                ...
                                                None,
                                                chat_id=chat_id)
        if waiting_for and waiting_for.startswith("edit_param_"):
        ...
        # 返回服务商编辑菜单
        if provider_id and provider_id in _state["providers"]:
            await show_provider_edit_menu(update, context, provider_id)
        else:
            # 如果无法确定服务商ID，返回主菜单
            await show_config_main_menu(update, context)
...
async def show_current_config(update: Update,
    ...
    if not _state["providers"]:
        ...
        for provider_id, provider in provider_items[start_idx:end_idx]:
            ...
            config_text += f"{is_default}{is_configured}<b>{provider_id}</b>\n"
        ...
        # 在文本中添加页码信息
        config_text += f"第 {page + 1}/{total_pages} 页"
...
async def handle_config_input(update: Update,
    ...
    # 处理不同类型的输入
    if waiting_for.startswith("provider_id_"):
        # 从等待状态中提取模板ID
        template_id = waiting_for.replace("provider_id_", "")

        # 处理新服务商 ID 输入
        provider_id = message_text.strip()

        # 验证 ID 格式
        if not re.match(r'^[a-zA-Z0-9]+$', provider_id):
            await message.reply_text("⚠️ 服务商 ID 只能包含字母和数字，请重新输入：")
            return
...
async def handle_group_config_message(
        update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """处理群聊中的配置相关消息（主要用于白名单配置）"""
    user_id = update.effective_user.id
    chat_id = update.effective_chat.id
