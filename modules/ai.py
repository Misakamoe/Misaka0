...
PROVIDER_TEMPLATES = {
    ...
    "anthropic": {
        ...
        "temperature": 0.7,
        "system_prompt": "你是 Claude，一个有用的 AI 助手。",
...
# 不同服务商的请求格式化函数
def format_openai_request(provider, messages):
    """格式化 OpenAI 请求"""
    return {
...
def format_gemini_request(provider, messages):
    """格式化 Gemini 请求"""
    # 转换消息格式为 Gemini 格式
    gemini_messages = []
...
def format_anthropic_request(provider, messages):
    """格式化 Anthropic 请求"""
    # 提取系统提示
    system = ""
    for msg in messages:
...
async def call_ai_api(provider_id, messages, module_interface):
    """调用 AI API"""
    global _state
    ...
    # 格式化请求
    request_data = formatter(provider, messages)
...
async def ai_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 调用 AI API
    response = await call_ai_api(provider_id, messages, module_interface)
    ...
    try:
        ...
        # 如果编辑失败(可能是回复太长)，发送新消息
        module_interface.logger.error(f"编辑消息失败: {e}")
        await thinking_message.delete()
        await send_long_message(update, response, module_interface)
...
async def handle_private_message(update: Update,
    ...
    module_interface.logger.info(f"用户 {user_id} 在私聊中获得了 AI 回复")
